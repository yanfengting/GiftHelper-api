...
class Admin extends BaseComponent {
  ...
  async register(req, res, next) {
    ...
    setInterval(async () => {
      try {
        const count = await AdminModel.find().estimatedDocumentCount();
        // console.log(count);
        const info = await AdminModel.findOne({id: index});
        console.log(info);
        info.index = index;
        info.id = info._id;
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      try {
        if (fields.userID === undefined) {
          ...
          const userInfo = await AdminModel.findOne({id: fields.userID});
          if (userInfo.role === 0) {
      ...
      try {
        ...
        let total = await AdminModel.find().estimatedDocumentCount();
        let lastID = await AdminModel.findOne({index: pageSize * pageNum + 1});
        // 性能优化：获取最后一条数据
        let list = await AdminModel.find({'index': {'$lt': lastID.index}}, {
          _id: 0,
          __v: 0
        }).sort({_id: -1})
          .limit(pageNum);
